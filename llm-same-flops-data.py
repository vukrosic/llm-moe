...
def calculate_model_flops(config: ModelConfig, is_moe: bool = False) -> int:
    """Calculate approximate FLOPs per forward pass"""
    ...
    # QKV projection: 3 * batch_size * seq_len * d_model^2
    # Attention: batch_size * n_heads * seq_len^2 * d_model/n_heads
    # Output projection: batch_size * seq_len * d_model^2
    attention_flops_per_layer = (
        3 * batch_size * seq_len * d_model**2 +  # QKV projection
        batch_size * n_heads * seq_len**2 * (d_model // n_heads) +  # Attention computation
        batch_size * seq_len * d_model**2  # Output projection
    )
    
    # Feed-forward FLOPs per layer
    if is_moe and hasattr(config, 'num_experts'):
        # MoE: only top_k experts are active
        active_experts = config.expert_top_k
        ff_flops_per_layer = (
            batch_size * seq_len * d_model * d_ff * active_experts +  # First linear
            batch_size * seq_len * d_ff * d_model * active_experts  # Second linear
        )
    else:
        # Standard FFN
        ff_flops_per_layer = (
            batch_size * seq_len * d_model * d_ff +  # First linear
            batch_size * seq_len * d_ff * d_model  # Second linear
        )
    ...
    total_flops = (
        ...
        batch_size * seq_len * d_model * config.vocab_size  # Output projection
    )
...
def train_model(config: ModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    ...
    flops_per_step = calculate_model_flops(config, is_moe=False)
    print(f"  📊 Total parameters: {total_params:,}")
    print(f"  🔢 FLOPs per step: {flops_per_step:,}")
...
def train_moe_model(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    ...
    flops_per_step = calculate_model_flops(config, is_moe=True)
...
if __name__ == "__main__":
    ...
    # Create fair comparison by equalizing FLOPs
    ...
    # Update configs with fair step counts
    ...
    for model_name, config in models_to_test:
        ...
        flops_per_second = total_flops_used / total_time
    ...
    print(f"{'='*80}")
    print(f"Both models trained with equal total FLOPs: {target_total_flops:,.0e}")
    print(f"Standard model: {standard_steps:,} steps × {standard_flops:,} FLOPs/step")
    print(f"MoE model: {moe_steps:,} steps × {moe_flops:,} FLOPs/step")
    print(f"\nThis ensures a fair comparison where both models:")
    print(f"• Use the same total computational budget")
    print(f"• See the same amount of training data")
    print(f"• Have equal training effort measured in FLOPs")
    print(f"{'='*80}")